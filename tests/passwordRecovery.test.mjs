...
test("el cambio usa la autorización CRM disponible en producción antes de actualizar", async () => {
  const source = await readFile("src/app/restablecer-clave/page.tsx", "utf8");
  assert.match(source, /auth\.getSession\(\)/);
  assert.match(source, /rpc\("is_crm_authorized"\)/);
  assert.match(source, /access\.data !== true/);
  assert.doesNotMatch(source, /get_crm_session_context/);
