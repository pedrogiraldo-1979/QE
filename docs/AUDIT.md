...
Ensayo: Pedro autorizó crear por $0/mes el proyecto temporal `QE Schema Baseline Temp 2026-09-25` (`hsuxurarysmvmprgdeqo`) en «Quindi Exquisito». Su referencia y su estado vacío se verificaron antes de aplicar el SQL. La aplicación terminó correctamente sólo allí. Se volvieron a comprobar cero filas en las diez tablas CRM, cero membresías en `private.crm_authorized_users` y cero usuarios en `auth.users`. Se solicitó la pausa y Supabase confirmó el estado `INACTIVE`.

Comparación de catálogo con lectura de QE2026: coinciden 11 tablas, 140 columnas, 36 restricciones, 37 índices, 22 funciones, 25 políticas, 172 privilegios de tabla y 55 privilegios de rutina, según las consultas de este ensayo. Las 465 firmas examinadas coinciden salvo diez hashes de texto de función; al normalizar comentarios y espacios, nueve son iguales y la única diferencia semántica restante es `public.get_cu_pending_reviews()`, precisamente por los dos filtros literales excluidos. Los tipos TypeScript generados desde ambos proyectos coinciden exactamente tras normalizar saltos de línea. Esta equivalencia estructural no demuestra por sí sola paridad de comportamiento con datos, autenticación o integraciones externas.

Asesores: el proyecto temporal reportó dos avisos informativos de [RLS habilitado sin política](https://supabase.com/docs/guides/database/database-linter?lint=0008_rls_enabled_no_policy), dos advertencias de [funciones `SECURITY DEFINER` ejecutables por `anon`](https://supabase.com/docs/guides/database/database-linter?lint=0028_anon_security_definer_function_executable) y siete para [`authenticated`](https://supabase.com/docs/guides/database/database-linter?lint=0029_authenticated_security_definer_function_executable), con los mismos conteos observados en QE2026. En rendimiento se registró una [FK sin índice](https://supabase.com/docs/guides/database/database-linter?lint=0001_unindexed_foreign_keys) y 23 [índices sin uso observado](https://supabase.com/docs/guides/database/database-linter?lint=0005_unused_index); QE2026 mostró uno y 18 respectivamente, diferencia esperable por el historial de uso. Estas alertas no se corrigen ni se descartan en este PR y requieren evaluación independiente antes de una adopción operativa.

Verificación local: prueba contractual 4/4, suite completa 45/45, `pnpm typecheck`, `pnpm build` (11 rutas) y smoke HTTP 11/11 aprobados. Los diez archivos de migración existentes permanecen intactos. No se ejecutaron `db push`, `migration repair`, fixtures autenticados, envío de correo ni cambios productivos.
