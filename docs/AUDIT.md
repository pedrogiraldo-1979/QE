...
- Revalidación del SQL registrado, sin publicarlo ni ejecutar funciones: ocho pares locales/remotos coinciden después de normalizar comentarios y espacios (cuatro versiones incrementales iguales, tres pares con timestamp distinto y RBAC). Siete también coinciden como texto normalizado a LF; el octavo, `20260720031715`, sólo añade un comentario local. RBAC coincide textualmente con el archivo local de SHA-256 `E3333A7D9C394A6ADABB3E41A4D51BE5791A0BAF920B0797A5B8DDD45135FE1E`. La baseline remota es un marcador de 79 caracteres frente a un archivo local estructural, y la allowlist remota contiene una siembra de membresías retirada del archivo local; no se imprimieron identificadores ni correos.
- En las 19 entradas sólo remotas, una lectura del SQL histórico encontró tres actualizaciones de filas de nivel superior: `20260707013441` normalizó campos de `contacts`, `20260722034019` rellenó `campaign_pilot_recipients.batch_key` antes de exigirlo como no nulo y `20260722142520` reconcilió `cu_responses.confirm_no_changes` con su payload. Las instrucciones `INSERT`/`UPDATE` dentro de funciones SQL no deben confundirse con DML ejecutado al aplicar la migración. No se consultaron ni alteraron las filas actuales.
...
### Ensayo estructural aislado autorizado — 2026-09-25

Pedro aprobó un proyecto temporal nuevo en la organización «Quindi Exquisito» tras confirmar el costo indicado por Supabase ($0/mes). Se creó `QE Migration Reconciliation Temp 2026-09-25` (`ludafyflotobvmowlmej`, región `ca-central-1`); su ref y nombre se comprobaron dos veces y son distintos de QE2026 (`izbfawwmbilmsrdjaanw`). Antes de aplicar SQL tenía cero tablas CRM, cero usuarios Auth y ningún historial de migraciones. Los primeros ocho archivos locales se aplicaron íntegros en orden. De `20260826000000_add_prospect_campaign_targeting.sql` se aplicaron exclusivamente la columna `prospects.campaign` y su índice, no las inserciones de 31 colegios/contactos. Finalmente se aplicó íntegra la migración RBAC de SHA-256 `E3333A7D9C394A6ADABB3E41A4D51BE5791A0BAF920B0797A5B8DDD45135FE1E`. El historial temporal registra diez pasos generados por el conector, pero el noveno está explícitamente marcado `schema_only`: no representa una reproducción literal de ese archivo.

Postflight temporal: cero filas en las diez tablas CRM, cero membresías y cero usuarios Auth. El catálogo comparado con producción mostró 11 tablas iguales en nombre/RLS, 25 políticas idénticas y 95 grants de tabla idénticos. El temporal tiene 139 columnas frente a 140 productivas; falta `campaign_pilot_recipients.batch_key`. Hay dos unicidades simples del lote en temporal donde producción usa dos compuestas por `batch_key`; el check de secuencia es 1–5 frente a 1–100. Los índices de lote correspondientes difieren (36 temporales, 37 productivos). De 22 funciones productivas, el temporal tiene 21: falta `claim_campaign_batch`. Entre las 21 compartidas, cuatro definiciones siguen distintas tras normalizar comentarios y espacios (`get_cu_form`, `submit_cu_form`, `get_cu_pending_reviews`, `claim_campaign_pilot_batch`); otras cuatro diferencias de hash se reducen a comentarios/formato. Los grants de funciones compartidas coinciden. No hay vistas, triggers de usuario ni secuencias en ninguno de los dos esquemas inspeccionados.
