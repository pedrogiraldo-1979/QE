...
const BATCH_KEY = "wave-2026-08-04-13";
...
export default function CampaignBatchPage() {
  ...
  async function sendBatch() {
    ...
    if (error) {
      setResult({
        ok: false,
        text: "El envío no confirmó un resultado completo. Revisa los estados antes de intentar otra acción.",
      });
      setSending(false);
    ...
    setResult({
      ...
      text: response?.message || response?.error || "ZeptoMail no confirmó el lote.",
    });
    setSending(false);
    await loadPreview(true);
  }
  ...
  if (!sessionReady) {
    return <Centered title="Validando sesión" description="Estamos comprobando que tu usuario esté autorizado." />;
  }
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ee] px-4 py-8 text-[#243126]">
      <section className="mx-auto max-w-5xl rounded-[1.75rem] border border-[#d8d2c7] bg-white p-6 shadow-sm sm:p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-[#687368]">Vista previa cerrada para 13 correos del lote {BATCH_KEY}. Los lotes anteriores permanecen separados y no pueden reenviarse.</p>
        <div className="mt-6 overflow-hidden rounded-2xl border border-[#d8d2c7]">
          ...
          {recipients.map((recipient) => (
            <article className="grid grid-cols-[48px_1fr_auto] gap-3 border-t border-[#e7e1d7] px-4 py-4 text-sm" key={recipient.id}>
              ...
              <div className="min-w-0"><p className="font-black">{recipient.companyName}</p><p className="mt-1 text-[#687368]">{recipient.recipientName}</p><p className="break-all text-[#687368]">{recipient.recipientEmail}</p><a className="mt-2 inline-block font-bold text-[#1f6b3a] underline" href={recipient.formUrl} target="_blank" rel="noreferrer">Revisar formulario</a></div>
        ...
        <div className="mt-6 space-y-4 rounded-2xl border border-amber-200 bg-amber-50 p-5">
          ...
          <label className="flex gap-3 text-sm font-bold text-amber-950"><input className="mt-0.5 h-5 w-5" type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} />Confirmo que revisé los 13 destinatarios y autorizo este lote.</label>
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
          ...
          <button className="rounded-xl bg-[#1f6b3a] px-5 py-3 text-sm font-black text-white disabled:opacity-50" type="button" disabled={!canSend} onClick={() => void sendBatch()}>{sending ? "Enviando lote…" : "Enviar 13 correos reales"}</button>
        </div>
      </section>
    </main>
  );
